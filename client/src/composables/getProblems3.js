  ...
  const load = async () => {
    ...
    try {
      const res = await getDocs(
        query(
          collection(projectFirestore, "problems"),
          orderBy("title"),
          limit(pagecount)
        )
      );
      ...
      const res2 = await getCountFromServer(
        collection(projectFirestore, "problems")
      );
    ...
    try {
      ...
      const res = await getDocs(
        query(
          ...
          endBefore(
            await getDoc(
              doc(projectFirestore, "problems", problems.value.at(0).id)
            )
          ),
    ...
    try {
      ...
      const res = await getDocs(
        query(
          ...
          startAfter(
            await getDoc(
              doc(projectFirestore, "problems", problems.value.at(-1).id)
            )
          ),
          limit(pagecount)
        )
